...
int main (int argc, char** argv) {
  ...
  hsize_t stride[2] = {block[0], block[1]};
  hsize_t Nlocal[2] = {block[0]/count[0], block[1]/count[1]};
  
  hsize_t offset[2] = {mpirank / dim[0], mpirank % dim[0]};
  for(int i=0; i<2; i++) offset[i] *= Nlocal[i];

  
  hid_t localspace = H5Screate_simple(2, block, NULL);
  H5Sselect_hyperslab(globalspace, H5S_SELECT_SET, offset, stride, count, Nlocal);
  H5Pclose(plist);
  vector<int> buffer(block[0]*block[1]);
  ...
  int sum = 0;
  for (int i=0; i<block[0]*block[1]; i++)
    sum += buffer[i];
  printf("sum=%d\n",sum);
